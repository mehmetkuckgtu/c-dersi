...
void labOn()
{
	...
	printf("uzantili dosya ektedir.");
	printf("\n************* part3.2 ****************\n");
	int part32=is_match(food1,part3);
	printf("\n%d\n",part32);
}
...
{
 char in_name[1000], /* strings giving names */
 out_name[1000]; /* of input and backup files */
 char ch; /* one character of input file */
	inp = fopen("food.txt", "r");
	outp =fopen("bin.bin","wb");
...
{
	int check=0;
	if(current_food.calories==parameters.proteinOnes || current_food.calories==parameters.proteinTwos)
	{
		check=1;
	}
	return check;
